...
describe('AuthModule', () => {
  ...
  it('should sign in with credentials and return user', async () => {
    ...
    // Verify POST /tokens was called with credentials
    const [url, options] = fetchMock.mock.calls[0];
    expect(url).toBe(`${IAM_URL}/api/v1/auth/tokens`);
    expect(JSON.parse(options.body)).toEqual({ email: 'user@test.com', password: 'pass' });
  });
  ...
  it('should refresh session and update tokens', async () => {
    ...
    // Verify refresh call
    const [url, options] = fetchMock.mock.calls[0];
    expect(url).toBe(`${IAM_URL}/api/v1/auth/tokens/refresh`);
