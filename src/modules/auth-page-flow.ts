...
interface AuthPageProviderProfile {
  ...
  /**
   * Set when the flow can also be finished in another tab. The message the
   * platform emails carries a link, and that link opens a tab which never saw
   * the popup this SDK opened, so the pending request is written even for a
   * popup and stays acceptable for this long.
   */
...
export class AuthPageFlow {
  ...
  async completeRedirect(): Promise<AuthTokenResponse | null> {
    ...
    if (context && this.hasExpired(context)) {
      ...
      throw new Error(`${this.providerLabel} sign-in request expired before it was completed.`);
    }
