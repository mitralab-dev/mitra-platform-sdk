...
`session({ create: true, agentType: "CUSTOM_AI", model })`, `agentTasks.create({ agentType: "CUSTOM_AI", model })` and `session.send(text, { agentType: "CUSTOM_AI", model })` pass `model` through unchanged: in the create body, in the message frame on the box socket and in the `POST /inputs` body. It is the catalog model id `agentCredentials.listModels()` returns, `custom/<providerId>/<model>` for a provider the person added by API, and the Copilot requires it when `agentType` is `CUSTOM_AI`. Those models carry `providerName`, the name the person chose for the provider; it is null on built-in models and absent on an older Copilot. A connection's custom providers, the ones an agent's business connection carries, are added server-side with the core SDK and this package does not expose them; the person's own custom providers are managed through `agentCredentials`, described further down in this section. Without `model` nothing changes on the wire.
