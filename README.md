...
Create a live task lazily on the first message. `agentId` names the business agent the chat belongs to; without it the chat is a plain Copilot chat:
...
A business agent's chat (`create: true` with an `agentId`) is born on the T3 box (`runtime: "T3"` in the create request) whenever the session can reach the box, over any transport, `http` included, so the first open does not wait for the box to adopt it. A chat without `agentId` is created without `runtime` and never asks for the direct channel: the Copilot refuses T3 outside an agent chat (`RUNTIME_REQUIRES_AGENT_APP`). Pass `runtime: "RUNNER"` or `runtime: "T3"` to decide explicitly.
...
The subscription usage, the meters behind the bar the IDE shows for a Claude or ChatGPT login, is readable with no chat open: `usage("ANTHROPIC")` (or `"OPENAI"`, with the same trailing `options`) resolves to `{ harness, observedAt, status, windows: [{ kind, usedPercent, resetsAt, windowSeconds }] }`, the last reading a chat on that credential reported, or `null` while none has. `windows` carries every window the provider reports (Claude: `FIVE_HOUR`, `WEEKLY`, `WEEKLY_OPUS` and any other it sends; Codex: primary and secondary, named by duration) and `status` is the provider's own (Claude: `allowed`, `allowed_warning`, `rejected`). Only a turn on the person's own subscription login, or the agent connection's, reports a window; an API key, a custom provider and the included AI never do. During a turn on the box the session emits the same reading live, in the same shape, as `providerUsage`, and the Copilot keeps it for the next `usage()`:

```typescript
session.on("providerUsage", ({ status, windows }) => console.log(status, windows))
const usage = await mitra.agentCredentials.usage("ANTHROPIC")
```
